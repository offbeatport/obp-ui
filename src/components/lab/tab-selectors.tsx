...
// ============================================================================
// 10 fancier tab-select variants for the company page. Each is a controlled
// component over the same contract, so any one can drop into $slug.tsx.
// ============================================================================
...
// ------------------------------------------------------------- 1. Underline slide
function UnderlineSlide({ tabs, active, onSelect, badges }: TabSelectorProps) {
...
// ------------------------------------------------------------- 2. Segmented pill
function SegmentedPill({ tabs, active, onSelect, badges }: TabSelectorProps) {
    const i = tabs.indexOf(active);
    const { ref, box } = useSlider(i);
    return (
        <div ref={ref} className="relative inline-flex gap-1 rounded-full border border-border bg-secondary/70 p-1">
            <span
            ...
            {tabs.map((t) => (
                <button
                    ...
                    className={cn(
                        ...
                        t === active ? "font-semibold text-foreground" : "font-medium text-faint hover:text-foreground",
                    )}
                >
                    {t}
                    <Badge v={badges?.[t]} />
...
// ------------------------------------------------------------- 3. Icon pills
function IconPills({ tabs, active, onSelect, badges }: TabSelectorProps) {
    return (
...
// ------------------------------------------------------------- 4. Command bar
function CommandBar({ tabs, active, onSelect, badges }: TabSelectorProps) {
    return (
        <div className="inline-flex items-center gap-1 rounded-xl border border-border bg-card p-1.5 shadow-e1">
            <span className="px-2 font-mono text-[10px] uppercase tracking-[0.16em] text-faint">⌘</span>
            {tabs.map((t, idx) => {
                const on = t === active;
                return (
                    <button
...
// ------------------------------------------------------------- 5. Vertical rail
function VerticalRail({ tabs, active, onSelect, badges }: TabSelectorProps) {
    const i = tabs.indexOf(active);
    const { ref, box } = useSlider(i);
    return (
        <div
...
// ------------------------------------------------------------- 6. Floating dock
function FloatingDock({ tabs, active, onSelect, badges }: TabSelectorProps) {
...
// ------------------------------------------------------------- 7. Numbered ticker
function NumberedTicker({ tabs, active, onSelect, badges }: TabSelectorProps) {
    const i = tabs.indexOf(active);
    const { ref, box } = useSlider(i);
    return (
...
// ------------------------------------------------------------- 8. Glow tabs (dark)
function GlowTabs({ tabs, active, onSelect, badges }: TabSelectorProps) {
    const i = tabs.indexOf(active);
    const { ref, box } = useSlider(i);
    return (
            ...
            <div ref={ref} className="relative flex gap-1">
                <span
                    ...
                    style={{
                        ...
                        boxShadow: "0 0 22px -4px rgba(150,130,255,0.8), inset 0 0 0 1px rgba(150,130,255,0.4)",
                    }}
                />
                {tabs.map((t) => {
                    const on = t === active;
                    return (
                        <button
                            ...
                            className={cn(
                                ...
                                on ? "font-semibold text-white" : "font-medium text-[#8790b8] hover:text-[#c9cfe8]",
                            )}
                        >
                            {t}
                            <Badge v={badges?.[t]} />
...
// ------------------------------------------------------------- 9. Morphing dropdown
function MorphDropdown({ tabs, active, onSelect, badges }: TabSelectorProps) {
    const Icon = ICONS[active] ?? LayoutGrid;
    return (
        <DropdownMenu>
                ...
                <div className="grid grid-cols-2 gap-1">
                    {tabs.map((t) => {
                        ...
                        return (
                            ...
                            >
                                <I
                                    className={cn("size-4", on ? "text-accent-foreground" : "text-muted-foreground")}
                                    strokeWidth={2}
                                />
...
function StatusTabs({ tabs, active, onSelect, badges }: TabSelectorProps) {
    return (
...
export type TabSelectorDef = {
    ...
    name: string;
    blurb: string;
    Component: (p: TabSelectorProps) => React.ReactNode;
