            ...
            >
                <div
                    className={cn(
                        ...
                        // entrance: plays once on mount (stable message-id key → no replay on polls)
                        me
                ...
                <div className="pt-0 px-[2px] pb-[2px]">
                    ...
                    <div className="mt-[12px]" hidden={!open}>
                        <p className="mb-[12px] text-[12.5px] leading-[1.5] text-muted-foreground">
                            Every opportunity it returns is scored 0–10 on these signals (willingness-to-pay counts
                            double):
                        </p>
                                ...
                                return (
                                    <div key={k} className="flex items-start gap-[8px] min-w-0 p-2">
                                        ...
                                        <span className="min-w-0">
                                            ...
                                            <span className="block text-xs leading-[1.4] text-faint">{meta.hint}</span>
                                        </span>
                                    </div>
                                );
                            })}
                                ...
                                <div className="flex items-stretch">
                                    <button
                                        className={cn(
                                            ...
                                            isOpen ? "bg-secondary" : chosen ? "bg-transparent" : "hover:bg-secondary",
                                        )}
                                        type="button"
...
function SpecLabel({ children }: { children: ReactNode }) {
    return <span className="font-mono text-[11px] font-bold uppercase tracking-[0.07em] text-faint">{children}</span>;
}
function SpecSection({ label, body }: { label: string; body?: string }) {
    ...
    return (
        <div className="flex flex-col gap-[5px]">
            ...
            <p className="text-[13.5px] leading-[1.58] text-muted-foreground whitespace-pre-line">{body}</p>
        </div>
    );
}
        ...
        <Dialog>
            ...
            <DialogContent className="max-w-[min(760px,92vw)] max-h-[86vh] overflow-y-auto gap-0 p-0">
                <div className="sticky top-0 z-10 bg-background border-b border-border px-[26px] pt-[24px] pb-[15px]">
                    ...
                    {c.wedge && <p className="mt-[6px] text-[14px] text-muted-foreground leading-[1.5]">{c.wedge}</p>}
                    <DialogDescription className="sr-only">Full opportunity spec for {c.name}</DialogDescription>
                </div>
                        ...
                        <div className="flex flex-col gap-[9px]">
                            ...
                            <div className="border border-border-soft rounded-[10px] overflow-hidden">
                                ...
                                {c.competitors.map((comp) => (
                                    ...
                                    >
                                        <span className="font-medium text-foreground">{comp.tool}</span>
...
export function SpecView({ spin, onCreate, busy }: { spin: Spin; onCreate: () => void; busy: boolean }) {
    ...
    return (
        <AssistantTurn>
            ...
            <div className="max-w-[820px] mt-[18px] mx-auto flex flex-col gap-[14px]">
                ...
                <div className="flex items-end justify-end gap-[20px] flex-wrap bg-secondary border border-border rounded-[14px] py-[16px] px-[18px]">
                    {/* No "choose a different angle" button - the ranked opportunity list stays
                        visible above this spec (picked line marked); click another row to re-pick. */}
                    <button
