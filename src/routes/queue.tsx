    ...
    <AppShell active="portfolio">
      <div className="mx-auto max-w-3xl px-6 py-10">
        <header className="flex items-start justify-between gap-4">
          <div>
            <h1 className="font-display text-3xl font-light">Action Queue</h1>
        ...
        {state?.actions.length ? (
          <div className="flex flex-col gap-1.5">
            {state.actions.map((a) => (
              <Row key={a.id}>
                <Badge variant={VARIANT[a.status] ?? "neutral"} className="w-28 justify-start">
                  {a.status}
                </Badge>
        ...
        {state?.runs.length ? (
          <div className="flex flex-col gap-1.5">
            {state.runs.map((r) => (
              <Row key={r.id}>
                <Badge variant={VARIANT[r.status] ?? "neutral"} className="w-28 justify-start">
                  {r.status}
                </Badge>
                <span className="flex-1 truncate font-mono text-xs text-faint">{r.id}</span>
          ...
          {logs.length ? (
            logs.map((l, i) => (
              <div key={`${l.t}-${i}`} className={logClass(l)}>
                <span className="mr-2 text-faint">{new Date(l.t).toLocaleTimeString()}</span>
