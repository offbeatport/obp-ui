    ...
    <div className="mx-auto max-w-5xl px-6 py-12">
      <header className="mb-10 flex items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-5 max-w-xl font-serif text-[17px] italic text-muted-foreground">
            From a thought to bag. Warm, editorial, a little literary — a terracotta glow on paper.
          </p>
        </div>
      ...
      <Section title="Logo" hint="The Glow-C wordmark (V7)">
        ...
        <p className="mt-4 max-w-2xl text-sm text-muted-foreground">
          The <b>mark</b> (glowing C block) is theme-independent and reads on any surface — use it
          collapsed, at favicon scale, or on-dark. The full <b>wordmark</b> adapts: braces + text
          follow the foreground; keep the terracotta glow intact.
        </p>
      </Section>
      ...
      <Section title="Color" hint="Terracotta on paper + the status language">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <Swatch name="ink" hex="#2c2926" className="bg-foreground" fg="text-background" />
        </div>
      ...
      <p className="mt-14 border-t pt-6 text-center text-xs text-faint">
        Logo component: <code className="font-mono">src/components/logo.tsx</code> · tokens in{" "}
...
function Swatch({
  ...
  return (
    <div className={`flex h-16 flex-col justify-end rounded-lg border p-2 shadow-e1 ${className}`}>
